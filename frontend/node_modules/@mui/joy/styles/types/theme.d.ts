...
import { Breakpoints, Spacing, SxProps as SystemSxProps, SystemProps as SystemSystemProps } from '@mui/system';
...
declare type Split<T, K extends keyof T = keyof T> = K extends string | number ? {
    [k in K]: Exclude<T[K], undefined>;
} : never;
declare type ConcatDeep<T> = T extends Record<string | number, infer V> ? keyof T extends string | number ? V extends string | number ? keyof T : keyof V extends string | number ? `${keyof T}-${ConcatDeep<Split<V>>}` : never : never : never;
/**
 * Does not work for these cases:
 * - { borderRadius: string | number } // the value can't be a union
 * - { shadows: [string, string, ..., string] } // the value can't be an array
 */
declare type NormalizeVars<T> = ConcatDeep<Split<T>>;
...
export declare type ThemeCssVar = OverridableStringUnion<NormalizeVars<ThemeVars>, ThemeCssVarOverrides>;
...
export declare type SxProps = SystemSxProps<Theme>;
export declare type SystemProps = SystemSystemProps<Theme>;
export {};
